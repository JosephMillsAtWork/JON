...
class JonWan22Sampler:
    ...
    def sample(self,
        ...
        ns = "JonWan22Sampler"
        pbar = comfy.utils.ProgressBar(9)
        ...
        if clip is not None:
            try:
                send_status(ns, "Text Encode Wan 2.2 img2vid Detected")
        ...
        send_status(ns, "Text Encode Wan 2.2 img2vid Done")
        pbar.update(1)
        ...
        send_status(ns, "Wan 2.2 First Last Frame")
        ...
        send_status(ns, "Wan 2.2 Sampler Done")
        ...
        pbar.update(1)
        send_status(ns, "Wan 2.2 Create Video Done")
        ...
        if save_video:
            ...
            pbar.update(1)
            send_status(ns, "Wan 2.2 Save Video Done")

        # check if the save of the last image if so save it
        if save_last_img:
            send_status(ns, "Wan 2.2 Gathering last image")
            last_frame = image_result[-1:]
            ...
            pbar.update(1)
            send_status(ns, "Wan 2.2 Last Image Saved")

        if not ui_results:
            send_status(ns, "Providing preview of first frame")
            first_frame = image_result[0:1]
            preview_out = nodes.PreviewImage().save_images(
                images=first_frame,
                filename_prefix="JonPreview"
            )
            ui_results.update(preview_out.get("ui", {}))
            send_status(ns, "Wan 2.2 last image Done")
