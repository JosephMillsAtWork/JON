...
let CACHED_LORA_NAMES_MO = [];

app.registerExtension({
    name: "JosephOddNodes.LoRAModelOnlyChain",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "JonLoRAModelOnly") {

            const resp = await fetch("/object_info/LoraLoader");
            const data = await resp.json();
            if (data && data.LoraLoader && data.LoraLoader.input.required.lora_name)
                CACHED_LORA_NAMES_MO = data.LoraLoader.input.required.lora_name[0];
            ...
            const updateHiddenConfig = (node) => {
                        ...
                        if (w.name) {
                            ...
                            if (w.name.startsWith("lora_name_")) {
                                ...
                                if (node.inputs) {
                                    ...
                                    if (input)
                                        state[id].input_name = input.name;
                                }
                            }
                ...
                const addLoRASlot = function(node, index, savedName, savedEnabled) {
                    ...
                    const initialLabel = getCleanLabel(savedName || CACHED_LORA_NAMES_MO[0]);
                    ...
                    const combo = node.addWidget("combo", widgetName, savedName || CACHED_LORA_NAMES_MO[0], (value) => {
                    ...
                    const setSlotState = (enabled) => {
                        // Disable the Dropdown (dim it)
                        combo.disabled = !enabled;
                        // Keep Remove Button & Toggle ACTIVE
                        btn.disabled = false;
                ...
                nodeType.prototype.onNodeCreated = function () {
                    ...
                    this.addWidget("button", "Add Model LoRA", null, () => {
                        const uniqueID = Date.now().toString().slice(-6);
                        addLoRASlot(this, uniqueID, null, true); // Default Enabled
